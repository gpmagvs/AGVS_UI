<template>
  <div class="agv-status border my-1">
    <div class="title d-flex flex-row">
      ...
      <div class="text-danger" v-if="!IsVMSConnect">{{ $t('HomeView.AGVStatus.AGVStatus.vms_disconnect') }}</div>
    </div>
    <el-table v-if="DisplayMode == 'table'"
      ...
      :data="AGVDatas" size="large" height="93%" empty-text="尚無AGV" :row-class-name="connected_class" style="width:100%"
      row-key="AGV_Name" border @row-click="HandleRowClick">
      <!-- 模擬器 column -->
      ...
      <!-- 車輛名稱 column -->
      <el-table-column :label="$t('HomeView.AGVStatus.AGVStatus.vehicle-name')" prop="AGV_Name" align="center" min-width="185px" type="index">
        <template #default="scope">
          <div class="d-flex">
            ...
            <div class="d-flex flex-column flex-fill">
              ...
              <b-button v-if="!IsRunMode" class="w-20 my-1 mx-2"
              ...
              <el-popover width="150" placement="right">
                <template #reference>
                  <b-button v-if="!IsRunMode" class="w-20 my-1 mx-2" @click="ShowAGVChargeConfirmDialog(scope.row)"
                ...
                <template #default>
                  <div class="charge-button-container d-flex flex-column">
                    <b-button variant="warning" style="width: 100%; margin-bottom: 8px;"
                      @click="ShowAGVChargeConfirmDialog(scope.row)">一般充電</b-button>
                    <b-button variant="success" style="width: 100%; margin-bottom: 8px;"
                      @click="ShowAGVChargeConfirmDialog(scope.row, true)">深度充電</b-button>
                    <b-button variant="light" style="width: 100%; margin-bottom: 8px;"
                      @click="StopDeepCharge(scope.row.AGV_Name)">解除深度充電</b-button>
                  </div>
                </template>
              </el-popover>
            </div>
      ...
      <!-- 車輛狀態 column -->
      <el-table-column :label="$t('HomeView.AGVStatus.AGVStatus.vehicle-statuslist')" prop="AGV_Status" align="left" min-width="480">
        <template #default="scope">
          <div class="w-100" style="position: absolute; top:0;left:0">
            <el-row class="h-50">
              <el-col :span="3">
                <div class="tb-label h-100 border px-1 py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.status') }}</div>
              </el-col>
              <el-col :span="6">
              ...
              <el-col :span="4">
                <div class="h-100 border py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.carrierid') }} </div>
              </el-col>
              <el-col :span="11">
            ...
            <el-row class="h-50">
              <el-col :span="3">
                <div class="tb-label h-100 border px-1 py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.vehicleconn') }}</div>
              </el-col>
              <el-col :span="6">
              ...
              <el-col :span="4">
                <div class="h-100 border py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.position') }} </div>
              </el-col>
              <el-col :span="11" class>
                <div class="w-100 h-100 border p-1 d-flex flex-row">
                  <div class="flex-fill">
                    <el-tooltip content="點一下跳至AGV當前位置" placement="top">
                      <i class="bi bi-geo-alt-fill" style="font-size:20px;cursor:pointer"
                  ...
                  <el-tooltip content="追隨AGV，AGV將保持顯示於地圖中央" placement="top">
                    <i v-if="!IsOpUsing" @click="TrackingVehicle(scope.row.AGV_Name)"
                      style="cursor: pointer;height: 22px;position: relative;top: 2px;border-radius: 30px;width: 22px;padding-left: 4px;font-size: 18px;"
                      class="bi bi-eyeglasses" v-bind:style="TrackingAGV == scope.row.AGV_Name ? {
                        backgroundColor: 'rgb(13, 110, 253)',
                        color: 'white'
                      } : {}"></i>
                  </el-tooltip>
                  <el-button class v-show="scope.row.Model == 2 || scope.row.Simulation"
            ...
            <el-row class="h-50">
              <el-col :span="3">
                <div class="tb-label h-100 border px-1 py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.power') }}</div>
              </el-col>
              <el-col :span="21">
                <div class="h-100 border px-2 py-1 d-flex">
                  <b-progress class="flex-fill" :max="100" :min="0" animated>
                    <i v-if="scope.row.IsCharging && IsVMSConnect"
                      v-bind:class="BatteryClass(scope.row.BatteryLevel_1, IsVMSConnect ? scope.row.IsCharging : false)"
                      style="color:white" class="bi bi-lightning-charge battery-icon"></i>
                    <b-progress-bar :animated="true"
                      v-bind:class="BatteryClass(scope.row.BatteryLevel_1, IsVMSConnect ? scope.row.IsCharging : false)"
                  ...
                  <b-progress v-if="scope.row.BatteryLevel_2 != -1.0" class="flex-fill mx-1" :max="100" :min="0"
                    animated>
                    <i v-if="scope.row.IsCharging && IsVMSConnect"
      ...
      <!-- 任務狀態 column -->
      <el-table-column v-if="!IsOpUsing" :label="$t('HomeView.AGVStatus.AGVStatus.task-status')" prop="Task_Status" align="left" min-width="380">
        <template #default="scope">
          <div class="w-100" style="position: absolute; top:0;left:0">
            <el-row class="h-50">
              <el-col :span="4">
            ...
            <el-row class="h-50">
              ...
              <el-col :span="4">
                <div class="tb-label h-100 border px-1 py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.From') }}</div>
              </el-col>
              <el-col :span="8">
                <div class="h-100 border p-1">
              ...
              <!-- Action Display -->
              <el-col :span="4">
                <div class="tb-label h-100 border px-1 py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.action') }}</div>
              </el-col>
              <el-col :span="8">
                <div class="h-100 border p-1 d-flex">
                  <el-tag class="h-100 w-100" effect="light">
                    <b class="val-label">{{ GetTransferProcessDescription(scope.row, scope.row.TransferProcess, scope.row.CurrentAction, scope.row.TaskRunAction) }}</b>
                  </el-tag>
                </div>
              </el-col>
              <!-- Action Display End -->
            ...
            <el-row class="h-50">
              <!-- To Station Display -->
              <el-col :span="4">
                <div class="tb-label h-100 border py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.To') }} </div>
              </el-col>
              <el-col :span="8">
                <div class="h-100 border p-1">
                  <el-tag class="h-100 w-100" effect="light">
              ...
              <el-col :span="4">
                <div class="tb-label h-100 border px-1 py-2 text-center bg-light">{{ $t('HomeView.AGVStatus.AGVStatus.Time') }}</div>
              </el-col>
              <el-col :span="8">
    ...
    <div v-if="DisplayMode == 'cards'" style="height:90vh;overflow-y: auto;" class="py-1">
      <vehicle-info-card v-for="vehicleState in AGVDatas" :key="vehicleState.AGV_Name" :vehicleStateData="vehicleState" :isMapViewTracking="TrackingAGV == vehicleState.AGV_Name" @OnOnlineBtnClicked="(vstate) => {
        ShowOnlineStateChangeModal(vstate.AGV_Name, vstate.OnlineStatus, vstate.Model)
      }" @OnLocateClicked="(agvName) => { HandleShowAGVInMapCenter(agvName) }" @OnTagLocatingClicked="(vstate) => { HandleAGVLocatingClick(vstate) }" @OnTrackingVehicleClick="(agvName) => { TrackingVehicle(agvName) }" @OnWebSiteIconClicked="(vstate) => { HandleAGVNameClicked(vstate) }"
        @ShowAGVChargeConfirmDialog="(vstate) => ShowAGVChargeConfirmDialog(vstate)" @ShowAGVDeepChargeConfirmDialog="(vstate) => ShowAGVChargeConfirmDialog(vstate, true)" @StopDeepCharge="(agvName) => StopDeepCharge(agvName)"
        @onTaskIdClick="(taskid) => HandleTaskIdClick(taskid)"></vehicle-info-card>
  ...
  methods: {
    ...
    ShowOnlineStateChangeModal(agv_name, current_online_status, Model) {
      ...
      if (!this.IsLogin) {
        this.$swal.fire(
          {
            text: '',
            title: this.$t('NoPermissionToChangeOnlineStatus'),
            icon: 'warning',
            showCancelButton: true,
    ...
    ShowAGVChargeConfirmDialog(agv_status, deepCharge = false) {
      ...
      if (userStore.getters.level < 0) {
        this.$swal.fire({
          title: this.$t('InsufficientPermissions'),
          text: this.$t('NoChargePermissionPleaseLogin'),
          icon: 'warning',
          showCancelButton: true
      ...
      const _chargeTaskConfirmMessage = (deepCharge) => {
        if (isInspectionAGV)
          ...
          return deepCharge ? `確定要將 ${agv_status.AGV_Name} 派送至充電站進行深度充電?` : `${agv_status.AGV_Name} `
      }
      this.$swal.fire(
        {
