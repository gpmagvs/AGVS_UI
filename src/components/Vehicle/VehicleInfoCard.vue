  ...
  <div id="vehicle-info-card" class="vehicle-info-card" :class="[vehicleStateData.Connected ? 'connected' : 'vehicle-card-disconnect', vehicleStateData.OnlineStatus == 0 ? 'v-Offline' : 'v-Online', IsOrderRunning ? 'order-run' : 'no-order-run'
    ...
    <div class="vehicle-info-card-header p-2 border-bottom">
      <el-tooltip :content="vehicleStateData.Model == 0 ? 'Forklifter AGV' : 'Submarine AGV'" placement="bottom-start">
        <div id="vehicle-info-card-icon" class="vehicle-icon"
          :class="vehicleStateData.Model == 0 ? 'fork-icon' : 'agv-icon-1'" :style="StyleOfAGVDisplayColor"></div>
      </el-tooltip>
      <h6 id="vehicle-info-card-name" class="flex-fill"
      ...
      <el-button v-if="vehicleStateData.Model == 2 || IsEmulationMode" class="text-primary" text
        @click="() => { $emit('OnTagLocatingClicked', vehicleStateData) }">定位</el-button>
      <el-tooltip content="點擊顯示車載頁面" placement="top">
        <i id="vehicle-info-card-web-site-icon" class="bi bi-globe mx-2" style="cursor:pointer;"
          @click="() => { $emit('OnWebSiteIconClicked', vehicleStateData) }"></i>
      </el-tooltip>
      <el-button id="vehicle-online-button"
        :class="vehicleStateData.OnlineStatus == 0 || !vehicleStateData.Connected ? 'text-danger' : 'text-success'"
        style="font-size:16px" @click="HandleOnlineButtonClicked">
        <template #icon>
    ...
    <div class="center-content d-flex justify-content-between p-2 m-1">
      ...
      <div id="vehicle-info-card-task-id">
        ...
        <el-tooltip content="點擊顯示任務詳情" placement="right" effect="light">
          ...
          <div class="text-truncate text-bold task-id" @click="HandleTaskIDClicked" style="width: 120px">{{
    CurrentOrderID
  }}</div>
        </el-tooltip>
      </div>
    ...
    <div class="footer">
      <div id="vehicle-info-card-from-station" class="button-like-container w-100"
      ...
      <div id="vehicle-info-card-to-station" class="button-like-container w-100 mx-1"
        :class="IsOrderRunning ? 'order-run' : 'no-order-run'">
        <div class="w-100 d-flex">
      ...
      <div id="vehicle-info-card-battery-info" class="button-like-container battery-info"
      ...
      <div class="button-like-container mx-1 d-flex" style="width: 200px;padding-right: 0px;">
        <el-button id="charge-task-assignment-button" type="danger" v-bind:class="getChargeButtnClass()"
        ...
        <el-popover width="150" placement="bottom">
          ...
          <template #default>
            <div class="charge-button-container d-flex flex-column">
              <b-button variant="warning" style="width: 100%; margin-bottom: 8px;font-size: 12px;"
                @click="() => $emit('ShowAGVChargeConfirmDialog', vehicleStateData)">{{ $t('NormalCharge')
                }}</b-button>
              <b-button variant="success" style="width: 100%; margin-bottom: 8px;font-size: 12px;"
                @click="() => $emit('ShowAGVDeepChargeConfirmDialog', vehicleStateData)">{{ $t('DepCharge')
                }}</b-button>
              <b-button variant="danger" style="width: 100%; margin-bottom: 8px;font-size: 12px;"
                @click="() => $emit('StopDeepCharge', vehicleStateData.AGV_Name)">{{ $t('CancelDeepCharge')
                }}</b-button>
            </div>
          </template>
        </el-popover>
