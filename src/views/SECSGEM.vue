    ...
    <div class="w-100 d-flex">
      <div class="main-content w-100 h-100 bg-light border rounded-3 mx-2">
        ...
        <div class="card platform-running-state d-flex">
          <h4>平台運行狀態</h4>
          <el-tag size="large" effect="dark" :type="isSecsPlatformRunning ? 'success' : 'danger'" style="height: 40px;">
            <span style="font-size: 1.2em;  font-weight: bold">{{ isSecsPlatformRunning ? '系統運行正常' : '系統運行異常' }}</span>
          </el-tag>
        </div>
        <div class="d-flex gap-2">
          <div class="card gem-connection-status">
            ...
            <div class="d-flex flex-column">
              <div class="text-start">
                <label>MCS</label>
                <el-tag effect="dark" :type="connectionStatusMap[connectionState.mcs].type"
                  style="width: 120px; height: 30px; text-align: center">{{
            connectionStatusMap[connectionState.mcs].text }}</el-tag>
              </div>
              <div class="text-start">
                <label>CIM</label>
                <el-tag effect="dark" :type="connectionStatusMap[connectionState.cim].type"
                  style="width: 120px; height: 30px; text-align: center">{{
          ...
          <div class="card gem-connection-status">
            ...
            <div class="d-flex flex-column align-items-center">
              <el-button :type="isSimulation ? 'primary' : 'danger'" size="large"
                style="width: 120px; height: 50px; font-size: 1.2em; font-weight: bold" @click="() => {
            $swal.fire({
          ...
          }" :title="isSimulation ? '關閉模擬' : '按一下已開啟模擬模式'">{{ isSimulation ? '模擬運行中' : '已關閉' }}</el-button>
            </div>
          </div>
        </div>
      </div>
